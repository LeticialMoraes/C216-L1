...
async def criar_aluno(payload: AlunoCreate) -> AlunoResponse:
    return await aluno_service.criar(payload)


@router.get("/", response_model=list[AlunoResponse])
async def listar_alunos() -> list[AlunoResponse]:
    return await aluno_service.listar()


@router.get("/{aluno_id}", response_model=AlunoResponse)
async def obter_aluno(aluno_id: str) -> AlunoResponse:
    try:
        return await aluno_service.obter(aluno_id)
    except AlunoNaoEncontradoError:
        raise _nao_encontrado() from None


@router.patch("/{aluno_id}", response_model=AlunoResponse)
async def atualizar_aluno(aluno_id: str, payload: AlunoUpdate) -> AlunoResponse:
    try:
        return await aluno_service.atualizar(aluno_id, payload)
    except AlunoNaoEncontradoError:
        raise _nao_encontrado() from None
...
@router.delete("/")
async def limpar_todos_alunos() -> dict[str, str]:
    return await aluno_service.limpar_todos()


@router.delete("/{aluno_id}", status_code=status.HTTP_204_NO_CONTENT)
async def remover_aluno(aluno_id: str) -> None:
    try:
        await aluno_service.remover(aluno_id)
    except AlunoNaoEncontradoError:
        raise _nao_encontrado() from None
